...
int plic_cold_irqchip_init(struct plic_data *plic)
{
	...
	if (plic->flags & PLIC_FLAG_ENABLE_PM) {
		...
		sbi_for_each_hartindex(hi) {
			if (plic->context_map[hi][PLIC_S_CONTEXT] < 0)
				continue;
	...
	if (!(plic->flags & PLIC_FLAG_NO_PRIORITY_INIT)) {
		for (int src = 1; src <= plic->num_src; src++)
			plic_set_priority(plic, src, 0);
	}
	...
	sbi_for_each_hartindex(hi) {
		if (plic->context_map[hi][PLIC_M_CONTEXT] < 0 &&
		    plic->context_map[hi][PLIC_S_CONTEXT] < 0)
			continue;

		plic_set_hart_data_ptr(sbi_hartindex_to_scratch(hi), plic);
	}
