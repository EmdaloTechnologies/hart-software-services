...
#define CONSOLE_HART_MAX 5

static const struct sbi_console_device *console_dev = NULL;
static char console_tbuf[CONSOLE_HART_MAX][CONSOLE_TBUF_MAX];
static u32 console_tbuf_len[CONSOLE_HART_MAX];
		...
		**out = '\0';
		if (out_len) {
			...
			if ((flags & USE_TBUF) && *out_len == 1) {
				unsigned long hid = csr_read(CSR_MHARTID);
				if (hid >= CONSOLE_HART_MAX) hid = 0;
				nputs_all(console_tbuf[hid], CONSOLE_TBUF_MAX - *out_len);
				*out = console_tbuf[hid];
...
static int print(char **out, u32 *out_len, const char *format, va_list args)
{
	...
	/*
	 * Per-hart console_tbuf avoids lock contention between harts.
	 */
	if (use_tbuf) {
		unsigned long hid = csr_read(CSR_MHARTID);
		if (hid >= CONSOLE_HART_MAX) hid = 0;
		console_tbuf_len[hid] = CONSOLE_TBUF_MAX;
		tout = console_tbuf[hid];
		out = &tout;
		out_len = &console_tbuf_len[hid];
	}
...
literal:
	...
	if (use_tbuf) {
		unsigned long hid = csr_read(CSR_MHARTID);
		if (hid >= CONSOLE_HART_MAX) hid = 0;
		if (console_tbuf_len[hid] < CONSOLE_TBUF_MAX)
			nputs_all(console_tbuf[hid], CONSOLE_TBUF_MAX - console_tbuf_len[hid]);
	}
