...
/*
 * HSS local patch: complete a HART_START from the requesting hart's context.
 *
 * When the target hart is relaunched via an out-of-band mechanism (e.g. HSS
 * IPI_MSG_GOTO) rather than going through the OpenSBI warmboot path,
 * sbi_hsm_hart_start_finish() is never called on the target.  This helper
 * performs the equivalent state transition (START_PENDING -> STARTED) and
 * releases the start ticket so that subsequent HART_START calls succeed.
 *
 * Must only be called after sbi_hsm_hart_start() has already moved the
 * target's state to START_PENDING.
 */
void sbi_hsm_hart_start_complete_for(struct sbi_scratch *rscratch)
{
...
static void sbi_hsm_hart_wait(struct sbi_scratch *scratch)
{
	...
	while (atomic_read(&hdata->state) != SBI_HSM_STATE_START_PENDING) {
		/*
		 * HSS local patch: do NOT call hsm_device_hart_stop() here.
		 *
		 * Upstream uses this for deeper power saving on platforms that
		 * can truly power-gate a core.  On MPFS, mpfs_hart_stop() does
		 * complex routing (j _start, jump_warmboot) only correct when
		 * called from sbi_hsm_exit() after an explicit HART_STOP ecall,
		 * not from the initial cold-boot wait loop where
		 * warmboot_addr is not yet initialised.
		 *
		 * Plain WFI is sufficient: MSIP/MEIP are enabled above and the
		 * hart will wake on the IPI sent by sbi_hsm_hart_start().
		 *
		 * Was:
